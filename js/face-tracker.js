 ...
 * Tracks user head roll angle, head position offset (X/Y), body movement,
 * and microphone speech volume to dynamically drive KuroBlob's avatar tilt,
 * jelly lean, eye gaze, and talking mouth in real-time.
 */
...
export class FaceTracker {
  ...
  async start(previewContainer) {
    ...
    try {
      // 1. Request Video & Audio (with fallback to video only if mic is rejected)
      try {
      ...
      // 3. Setup Audio Analyser for Lip-sync (if audio track exists)
  ...
  async trackLoop() {
    ...
    if (this.video.readyState >= 2) {
      ...
      if (this.analyser && this.audioData) {
        ...
        // Sensitivity curve
        const targetMouth = Math.min(1.0, Math.max(0, (avg - 12) / 45));
        this.mouthLevel += (targetMouth - this.mouthLevel) * 0.45;
      } else {
        this.mouthLevel = 0;
      }
        ...
        try {
          ...
          if (faces && faces.length > 0) {
            ...
            if (eyes.length >= 2) {
              ...
              rawRoll = -Math.atan2(dy, dx) * 1.8; // Amplified roll response
            } else {
              rawRoll = normX * 0.55;
            }
            faceDetected = true;
      ...
      // Responsive lerp (0.32) for instant real-time response
      this.smoothRoll += (rawRoll - this.smoothRoll) * 0.32;
      this.smoothBodyX += (rawBodyX - this.smoothBodyX) * 0.35;
      this.smoothBodyY += (rawBodyY - this.smoothBodyY) * 0.35;
      this.smoothOffsetX += (rawOffsetX - this.smoothOffsetX) * 0.35;
      this.smoothOffsetY += (rawOffsetY - this.smoothOffsetY) * 0.35;

      // Apply bold parameters directly to KuroBlob avatar
      if (this.avatar) {
        this.avatar.setFaceTracking(true, {
          roll: Math.max(-0.65, Math.min(0.65, this.smoothRoll)),
          bodyX: Math.max(-45, Math.min(45, this.smoothBodyX)),
          bodyY: Math.max(-35, Math.min(35, this.smoothBodyY)),
          offsetX: Math.max(-38, Math.min(38, this.smoothOffsetX)),
          offsetY: Math.max(-28, Math.min(28, this.smoothOffsetY)),
  ...
  computeOpticalTracking() {
    ...
    for (let y = 6; y < ch - 6; y += 3) {
      for (let x = 6; x < cw - 6; x += 3) {
        ...
        const centerWeight = Math.max(0.3, 1.4 - distFromCenter * 0.9);

        // Warm tone or prominent luminance detection
        const isWarm = (r > g && g > b && r > 40);
        const isBright = luma > 75;
    ...
    if (weightSum > 8) {
      ...
      if (this.baselineX === null) {
        ...
        this.baselineX += (avgX - this.baselineX) * 0.006;
        this.baselineY += (avgY - this.baselineY) * 0.006;
      }

      const diffX = (avgX - this.baselineX) / (cw * 0.25);
      const diffY = (avgY - this.baselineY) / (ch * 0.25);

      const bodyX = Math.max(-45, Math.min(45, diffX * 45));
      const bodyY = Math.max(-35, Math.min(35, diffY * 35));
      const offsetX = Math.max(-38, Math.min(38, diffX * 38));
      const offsetY = Math.max(-28, Math.min(28, diffY * 28));

      const lumaDiff = (leftCount && rightCount) ? ((rightLuma / rightCount) - (leftLuma / leftCount)) / 80 : 0;
      const roll = Math.max(-0.65, Math.min(0.65, (diffX * 0.55) + (lumaDiff * 0.3)));
  ...
  renderPreview(bodyX, bodyY, roll, mouthLevel) {
    ...
    this.ctx.shadowColor = 'rgba(0, 242, 254, 0.7)';
    this.ctx.shadowBlur = 8;

    const cx = cw / 2 + (bodyX / 45) * (cw * 0.35);
    const cy = ch / 2 + (bodyY / 35) * (ch * 0.35);
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(cx - Math.cos(roll) * 28, cy - Math.sin(roll) * 28);
    this.ctx.lineTo(cx + Math.cos(roll) * 28, cy + Math.sin(roll) * 28);
    this.ctx.stroke();

    // Mic Voice VU Meter bar
    if (mouthLevel > 0.05) {
      this.ctx.fillStyle = '#34C759';
      this.ctx.fillRect(8, ch - 12, Math.min(cw - 16, mouthLevel * (cw - 16)), 4);
    }
